...
class AaronsSpider(Spider):
    ...
    def make_request(self, offset: int, limit: int = 50) -> JsonRequest:
        return JsonRequest(
            url=f"https://www.aarons.com/locations/search-api?r=250000&per={limit}&offset={offset}",
    ...
    def parse(self, response: Response, **kwargs: Any) -> Iterable[Feature | JsonRequest]:
        for location in response.json()["response"]["entities"]:
            ...
            # c_storeCode is null on a handful of stores, and shared by three others
            item["ref"] = location.get("c_storeCode") or location["meta"]["id"]
